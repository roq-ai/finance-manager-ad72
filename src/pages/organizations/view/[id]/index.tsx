...
import { deleteBillingById } from 'apiSdk/billings';
...
function OrganizationViewPage() {
  ...
  const billingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('billing', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/billings/create?organization_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.billing?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/billings/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.cost}</Td>
